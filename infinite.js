...
$('.issuenumber').each(function(){
 ...
 .on("enter", function (event) { 
	...
	if (maptheme[triggeredissue].length > 5){
		textFit($('#issuethemedown'),{widthOnly: true})
	}
...
d3.selectAll("#pdfback").on("click", function() { 
	window.location = "#issues"
	pdfback()})
...
$(window).on('load', function(){
	window.setTimeout(hideloader(loadpage), 1000)
})
...
console.log("web dev by Karyn and Diego :)")
